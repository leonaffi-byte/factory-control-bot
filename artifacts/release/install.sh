...
if [[ -f "${ENV_FILE}" ]]; then
    ...
    echo ""
    echo -e "${BOLD}API keys (press Enter to skip any):${NC}"
    echo ""

    read -rp "  Groq API Key (voice transcription):    " KEY_GROQ
    read -rp "  OpenAI API Key:                        " KEY_OPENAI
    read -rp "  OpenRouter API Key (free models):      " KEY_OPENROUTER
    read -rp "  Google AI API Key (Gemini):             " KEY_GOOGLE
    read -rp "  NVIDIA NIM API Key:                    " KEY_NVIDIA
    read -rp "  Together AI API Key:                   " KEY_TOGETHER
    read -rp "  Cerebras API Key:                      " KEY_CEREBRAS
    read -rp "  SambaNova API Key:                     " KEY_SAMBANOVA
    read -rp "  Fireworks AI API Key:                  " KEY_FIREWORKS
    read -rp "  Mistral API Key:                       " KEY_MISTRAL
    read -rp "  Perplexity API Key:                    " KEY_PERPLEXITY
    read -rp "  GitHub Token (repo management):        " KEY_GITHUB
...
TOGETHER_API_KEY=${KEY_TOGETHER}
CEREBRAS_API_KEY=${KEY_CEREBRAS}
SAMBANOVA_API_KEY=${KEY_SAMBANOVA}
FIREWORKS_API_KEY=${KEY_FIREWORKS}
MISTRAL_API_KEY=${KEY_MISTRAL}
PERPLEXITY_API_KEY=${KEY_PERPLEXITY}
GITHUB_TOKEN=${KEY_GITHUB}
